...
const chartOptions = {
  ...
  scales: {
    y: {
      beginAtZero: false,
      // Add padding to the top of the scale for labels
      grace: '10%' 
    }
  },
  plugins: {
    ...
    // New datalabels config
    datalabels: {
      anchor: 'end',
      align: 'end',
      offset: -5, // Position labels just inside the top of bars/points
      font: {
        weight: 'bold',
        size: 10
      },
      color: (context) => {
        // *** FIX: Replaced Chart.js v2 helper with a static color for v4 ***
        return '#333';
      },
      formatter: (value, context) => {
        if (value === null || value === 'na' || isNaN(value)) return '';
        ...
        return new Intl.NumberFormat('en-SG', { 
          style: 'currency', 
          currency: 'SGD', 
          minimumFractionDigits: 2, // Changed from 0
          maximumFractionDigits: 2, // Changed from 0
          notation: 'compact' 
        }).format(value);
      },
      // Hide labels that are 0 or null
      display: (context) => {
...
<template>
  <div class="container py-5">
    ...
    <div v-else-if="latestRecord">
      ...
      <div v-if="isForecasting" class="text-center p-5">
            ...
            <div class="card h-100">
              <div class="card-body">
                ...
                <table class="table table-sm">
                  <thead>
                    <tr><th>Year</th><th class="text-end">Predicted Median Salary</th></tr>
                  </thead>
                  <tbody>
              ...
              <div class="card-body">
                ...
                <ul class="list-group list-group-flush">
                  ...
                  <li class="list-group-item">
                    ...
                    <div class="font-monospace small mt-1">Salary ≈ {{ forecastResponse.model_details.equation.intercept.toFixed(2) }} <br>
                      + ({{ forecastResponse.model_details.equation.coefficients.national_median.toFixed(2) }} * Nat. Median) <br>
                      + ({{ forecastResponse.model_details.equation.coefficients.unemployment.toFixed(2) }} * Unemployment) <br>
                      + ({{ forecastResponse.model_details.equation.coefficients.annual_gdp.toFixed(4) }} * GDP)
                    </div>
                  </li>
                </ul>
              </div>
